...
public class DrawTwoCardTest {
    ...
    void testPlay() {
        // +2 +2 +2 +4
        // TODO Implement here
        GameLogic gameInstant = GameLogic.getInstance(4);
        for (int i=0; i<10; i++)
            gameInstant.getDeck().add(new NumberCard(CardColor.randomColor(), CardSymbol.randomSymbol()));

        ArrayList<BaseCard> first = gameInstant.getPlayerHand(0);
        ArrayList<BaseCard> second = gameInstant.getPlayerHand(1);
        ArrayList<BaseCard> third = gameInstant.getPlayerHand(2);
        ArrayList<BaseCard> fourth = gameInstant.getPlayerHand(3);
        ...
        String message = gameInstant.getCurrentPlayerHand().get(0).play();
