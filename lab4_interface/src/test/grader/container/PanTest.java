...
class PanTest {
	...
	void testCook() {
		...
		Egg e = new Egg();
		test.setMaxCapacity(3);      //default Pan can hold only 1 ingredient so I expand it
		test.setCapacity(3);
		test.addContent(m);
		test.addContent(l);         // add lettuce (not cookable, which will not get added) and egg
		test.addContent(e);		
